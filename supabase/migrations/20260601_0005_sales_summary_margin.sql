-- =============================================================================
-- 20260601_0005 — Margen / rentabilidad en get_sales_summary_v2 (RF-R1)
-- =============================================================================
--
-- Cablea el COSTO real en el resumen de ventas. Hasta ahora `product_sales`
-- emitía `cost` y `gross_profit` hardcodeados en 0; ahora se calculan desde
-- `menu_items.cost` (costo ACTUAL del producto × cantidad vendida).
--
-- Cambios (todos ADITIVOS — ningún campo se quita ni se renombra):
--   1. scoped_items: nueva columna `unit_cost` = COALESCE(mi.cost, 0).
--      Se propaga sola por items_ranked/adjusted/allocated (SELECT *).
--   2. product_sales: `cost` y `gross_profit` reales + nuevo `margin_pct`.
--   3. sales_by_category: nuevos `cost`, `gross_profit`, `margin_pct`.
--   4. raíz: nuevos `total_cost`, `gross_profit_total`, `margin_pct_total`.
--
-- ⚠️ COSTO ACTUAL, NO HISTÓRICO: order_items NO snapshotea el costo, así que el
-- margen usa el costo vigente del producto. Aceptable para la mayoría de casos;
-- la UI muestra el disclaimer correspondiente. Snapshot histórico = decisión D-7.
-- Ver docs/SPEC_REPORTE_MARGEN.md.
--
