...
create table public.modifier_groups (
  id uuid not null primary key, business_id uuid not null, name text not null,
  min_select integer default 0 not null, max_select integer default 0 not null,
  is_active boolean default true not null, created_at timestamptz default now() not null,
  display_type text not null default 'multiple' check (display_type in ('single','multiple','toggle')),
  selection_mode text not null default 'modifier' check (selection_mode in ('modifier','extra','removal')),
  is_required boolean not null default false, free_qty integer not null default 0 check (free_qty >= 0),
  max_qty_per_option integer not null default 1 check (max_qty_per_option >= 1),
  sort_order integer not null default 0 check (sort_order >= 0));
create table public.modifiers (
  id uuid not null primary key, business_id uuid not null,
  group_id uuid references public.modifier_groups(id), name text not null,
  price_delta numeric(12,2) default 0 not null, is_active boolean default true not null,
  created_at timestamptz default now() not null, sku text,
  default_selected boolean not null default false, sort_order integer not null default 0);
create table public.menu_item_groups (
  menu_item_id uuid not null references public.menu_items(id), group_id uuid not null references public.modifier_groups(id),
  primary key (menu_item_id, group_id));
-- Réplica de prod según el diagnóstico del 2026-09-24.
insert into public.businesses (id, business_name, status) values ('d16497fa-4853-41e3-8566-4d0565511f37', 'MAX PICAME EL POLLO', 'active');
insert into public.business_settings (business_id, inventory_mode) values ('d16497fa-4853-41e3-8566-4d0565511f37', 'basic');
insert into public.taxes (business_id, name, rate, apply_on_quick, apply_on_delivery) values
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'ITBIS', 18, true, true),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'LEY', 10, false, false);
insert into public.print_areas (business_id, name, code) values
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'Caja', 'cashier'),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'Fiscal', 'fiscal'),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'Cierre de caja', 'cash_close'),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'BAR', 'bar'),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'CANTINA', 'cantina'),
  ('d16497fa-4853-41e3-8566-4d0565511f37', 'COCINA', 'cocina');
insert into public.print_area_printers select id, gen_random_uuid() from public.print_areas;
insert into public.menus (id, business_id, name) values (gen_random_uuid(), 'd16497fa-4853-41e3-8566-4d0565511f37', 'PRINCIPAL');
insert into public.categories (id, business_id, name, position) values
  (gen_random_uuid(), 'd16497fa-4853-41e3-8566-4d0565511f37', 'POLLO', 10),
  (gen_random_uuid(), 'd16497fa-4853-41e3-8566-4d0565511f37', 'PLATO FUERTE', 20);
insert into public.menu_items (business_id, category_id, name, price, tax_mode, print_area_code)
select 'd16497fa-4853-41e3-8566-4d0565511f37', c.id, v.n, v.p, 'inclusive', v.a
from (values ('Agua',25,'POLLO','bar'),('CHOFAN',100,'POLLO','cocina'),('Pechurina',200,'POLLO','cocina'),
             ('POLLO FRITO',350,'POLLO','cocina'),('DOBLE RESERVA',2000,'PLATO FUERTE','cantina')) v(n,p,c,a)
...
insert into public.menu_item_taxes select mi.id, t.id from public.menu_items mi, public.taxes t
 where t.name = 'ITBIS' or mi.name in ('Pechurina', 'DOBLE RESERVA');
insert into public.menu_item_print_areas (menu_item_id, print_area_id)
select mi.id, a.id from public.menu_items mi join public.print_areas a on a.code = mi.print_area_code;
