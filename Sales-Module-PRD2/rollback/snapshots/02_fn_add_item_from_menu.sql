...
-- Source: SELECT pg_get_functiondef('public.fn_add_item_from_menu'::regproc);
-- Use: rollback target if PRD 2 modifications break this function.

CREATE OR REPLACE FUNCTION public.fn_add_item_from_menu(p_order_id uuid, p_menu_item_id uuid, p_qty numeric DEFAULT 1, p_check_position integer DEFAULT 1, p_is_takeout boolean DEFAULT false, p_notes text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
