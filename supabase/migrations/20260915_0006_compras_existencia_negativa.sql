-- =============================================================================
-- 20260915_0006 — Compras: la existencia NEGATIVA cuenta como 0 en el pedido
--
-- PRD: docs/PRD_COMPRAS_PEDIDO_SUGERIDO.md (F1). REQUIERE: 20260915_0004.
--
-- POR QUÉ: la verificación en prod (2026-09-15) dio 126 insumos de La Penda con
--   existencia negativa en el Almacén Principal. Con la 0004,
--     sugerido = objetivo − (existencia + en camino + ya pedido)
--   y una existencia de −40 hacía pedir 40 de más para «tapar» un faltante que
--   físicamente no existe: un negativo es consumo cuya entrada no se registró
--   (compra sin digitar, conteo pendiente). En la bodega hay 0.
--
-- AHORA: disponible = max(existencia, 0) + en camino + ya pedido (también para
--   «días que alcanza»). La columna `stock` sigue devolviendo el negativo real
--   para que la pantalla lo avise. Corregir el negativo es del NEGOCIO (conteo,
--   compras sin registrar), no de esta función.
--
-- MISMA firma y MISMAS columnas que la 0004: create or replace, sin drop; la app
--   no cambia de contrato.
--
-- PRUEBA: supabase/tests/compras_f1_local_test.sh (P9 y rollback).
-- IDEMPOTENTE: sí. REVERSIBLE: sí (_ROLLBACK restaura la definición de la 0004).
-- =============================================================================
...
begin
  if not exists (
    ...
    raise exception 'Falta aplicar 20260915_0004 (fn_purchase_projection). No se aplicó nada.';
  end if;
end
$guard$;
...
as $$
  ...
  rates as (
    select b.*,
           ...
           -- Un negativo es consumo sin su entrada registrada (compra no digitada,
           -- conteo pendiente): físicamente hay 0, no se pide para «taparlo».
           greatest(b.stock, 0) + b.in_transit + b.on_order                                as available
      from base b
  ),
  projected as (
    select r.*,
...
comment on function public.fn_purchase_projection(uuid, uuid, integer, integer, integer, integer, uuid, boolean) is
  ...
  'fn_purchase_resolve_suppliers. Todo en unidad base. La existencia negativa '
  'cuenta como 0 (la columna stock la devuelve tal cual). Ver 20260915_0004 y 0006.';
