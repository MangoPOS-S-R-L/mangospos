-- =============================================================================
-- 20260915_0004 — Compras F1: el motor de proyección («cuánto pedir»)
--
-- PRD: docs/PRD_COMPRAS_PEDIDO_SUGERIDO.md (F1, decisiones D1/D2/D5/D9).
-- REQUIERE: 20260915_0003 (fn_purchase_resolve_suppliers).
--
-- QUÉ ENTREGA: `fn_purchase_projection`, SOLO LECTURA. Una fila por insumo
--   activo (sin servicios ni combos) con todo lo que la pantalla de pedido
--   necesita para decidir, en UNIDAD BASE:
--
--     consumo diario  = (ventas netas + mermas + salidas de producción)
--                       de la ventana ÷ días de la ventana                [D1]
--     objetivo        = consumo diario × (tiempo de entrega + cobertura)
--                       + mínimo (colchón de seguridad)                  [D2]
--     sugerido        = max(0, objetivo − existencia − en tránsito − ya pedido)
--     mínimo sugerido = consumo diario × (tiempo de entrega + días de colchón)
--
-- LO QUE NO CUENTA COMO CONSUMO (B6): transferencias (pasan por
--   __IN_TRANSIT__), ajustes y conteos, devoluciones a suplidor. La rotación
--   vieja sí los contaba y habría pedido de más en negocios con varias bodegas.
--   Las devoluciones de venta ('sale' positivo, anulaciones) SÍ restan: el
--   consumo es NETO.
--
-- LA VENTANA: los días con los que se calcula son los que el insumo tiene de
--   historia, hasta `p_days_back`. Un insumo que empezó a moverse hace 10 días
--   se divide entre 10, no entre 30 — si no, su consumo diario saldría a un
--   tercio y nunca aparecería en el pedido. La columna `window_days` lo dice.
--
-- POR ALMACÉN (D5): con `p_warehouse_id`, existencia, consumo, tránsito y
--   órdenes son de ESE almacén y el mínimo es el del almacén (si lo tiene; si
--   no, el del insumo). Sin almacén: todo el negocio, sin el de tránsito ni
--   los inactivos.
--
-- TIEMPO DE ENTREGA (D9): el del suplidor; si no lo tiene,
--   `p_default_lead_time_days`, y `lead_time_is_default` lo marca.
--
-- NEGOCIOS SIN INVENTARIO: la función no escribe nada y solo corre si alguien
--   la llama.
--
-- PRUEBA: supabase/tests/compras_f1_local_test.sh (Postgres 15 local).
