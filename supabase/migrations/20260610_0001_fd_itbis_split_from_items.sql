-- =============================================================================
-- Fix v5: fn_recompute_fd_for_scope — ITBIS/LEY del comprobante derivados de
-- los ÍTEMS (no de order.tax).
--
-- PROBLEMA (v4, 20260530_0012):
--   fiscal_documents.itbis_amount = order.tax. Pero order.tax:
--     - en mayo trae el 28% JUNTO (ITBIS+LEY) → ITBIS sobre-declarado.
--     - desde el 30-may queda en 0 (el trigger 20260530_0007 recalcula
--       calculate_order_totals en medio del split/check con ítems
--       transitorios) → ITBIS sub-declarado (~RD$0 en junio).
--   En ambos casos el comprobante nunca SEPARA ITBIS de LEY.
--
-- FIX (v5): el comprobante deriva sus montos de los ÍTEMS de su scope (orden
--   completa o el check específico), partiendo `oi.tax` (lo realmente cobrado)
--   según las tasas de la config del negocio:
--     - ITBIS  = impuesto(s) con taxes.is_service_fee = false  (ej. 18%)
--     - LEY    = impuesto(s) con taxes.is_service_fee = true   (ej. 10%)
--   Lee las tasas de `taxes` (no asume 18/10) → maneja agua 18%-solo,
--   combinados, solo-ley, exentos. Usa oi.tax (robusto: tax_rate=0 → 0,
--   esquiva los tax_lines inflados del bug de split histórico).
--
-- SEGURIDAD:
--   - Solo redefine ESTA función. No toca POS, órdenes, ítems, pagos ni la app.
--   - NO corre backfill: los comprobantes YA emitidos quedan intactos. Solo
--     afecta los recomputes NUEVOS (emisión/pago de aquí en adelante).
--   - Si un comprobante NO tiene ítems en su scope (manual/huérfano) → NO lo
--     toca (sale sin modificar).
--   - Conserva la lógica de prorrateo por pago parcial de v4.
--   - ROLLBACK disponible: 20260610_0001_fd_itbis_split_from_items_ROLLBACK.sql
-- =============================================================================
