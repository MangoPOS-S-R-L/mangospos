...
def _write_areas(items, n_kit, n_bar):
    ...
    w("--")
    w("-- CREA LAS DOS AREAS si no existen: `cocina` (Cocina) y `bar` (Barra),")
    w("-- y despues les asigna los %d productos. Si ya existen, las reusa tal cual." % (n_kit + n_bar))
    w("--")
    w("-- CANDADO: solo crea si el negocio NO tiene ninguna area activa. Si ya")
    w("--   tiene areas y no estan las dos mias, ABORTA y te las lista — incluso")
    w("--   si calza una sola: crear la que falta al lado de un `barra` que ya")
    w("--   hace ese papel deja 33 productos apuntando a un area nueva y vacia.")
    w("--   El code lo genera la app desde el nombre: Cocina -> cocina, Bar -> bar,")
    w("--   Barra -> barra, y `_2` si choca. Pon los reales en el bloque 1.")
    w("--")
    w("-- LO QUE NO HACE: pegarle una impresora a cada area. Un area sin impresora")
    w("--   acepta el producto pero la comanda NO sale. Eso se configura en")
    w("--   Ajustes -> Areas de impresion, o con el bloque comentado del final.")
    w("--")
    ...
    w("")
    w("-- ---------------------------------------------------------------------------")
    w("-- 1) LAS DOS AREAS — se CREAN si no existen. Cambia `name`/`code` aqui")
    w("--    si el negocio las llama de otra forma.")
    w("-- ---------------------------------------------------------------------------")
    w("create temp table _area_cfg (rol text primary key, code text, name text) on commit drop;")
    w("insert into _area_cfg (rol, code, name) values")
    w("  ('K', 'cocina', 'Cocina'),   -- area de COCINA")
    w("  ('B', 'bar',    'Barra');    -- area de BARRA")
    w("")
    w("do $$")
    w("declare")
    w("  v_business  uuid := '%s';" % BID)
    ...
    w("  v_creadas   text;")
    w("begin")
    w("  -- Reactiva primero: un area NUESTRA apagada cuenta como existente, si no")
    w("  -- rutearia a un area desactivada y el control daria sin_area_nm > 0.")
    w("  update public.print_areas a")
    w("     set is_active = true")
    w("    from _area_cfg cfg")
    w("   where a.business_id = v_business and a.code = cfg.code")
    w("     and not coalesce(a.is_active, true);")
    w("")
    w("  select count(*) into v_activas")
    w("  from public.print_areas a")
    w("  where a.business_id = v_business and coalesce(a.is_active, true);")
    w("")
    w("  select count(*) into v_nuestras")
    w("  from public.print_areas a")
    w("  join _area_cfg cfg on cfg.code = a.code")
    w("  where a.business_id = v_business and coalesce(a.is_active, true);")
    w("")
    w("  -- CANDADO. Solo hay dos caminos limpios: el negocio no tiene NINGUNA")
    w("  -- area activa (las creamos), o ya tiene LAS DOS nuestras (las reusamos).")
    w("  -- Cualquier mezcla aborta: crear la que falta al lado de un area que ya")
    w("  -- hace ese papel con otro nombre (Barra -> code `barra`) deja el ruteo")
    w("  -- partido y 33 productos apuntando a un area nueva y vacia.")
    w("  if v_activas > 0 and v_nuestras < (select count(*) from _area_cfg) then")
    w("    select string_agg(a.code || ' (' || a.name || ')', ', ' order by a.code)")
    w("      into v_otras")
    w("    from public.print_areas a")
    w("    where a.business_id = v_business and coalesce(a.is_active, true);")
    w("")
    w("    select string_agg(cfg.code, ', ' order by cfg.code)")
    w("      into v_faltan")
    w("    from _area_cfg cfg")
    w("    where not exists (")
    ...
    w("    );")
    w("")
    w("    raise exception 'El negocio ya tiene estas areas activas: %. De las mias faltan: %. Pon los codes REALES en el bloque 1 y vuelve a correr (el code lo genera la app desde el nombre: Cocina -> cocina, Barra -> barra).', v_otras, v_faltan;")
    w("  end if;")
    w("")
    w("  with ins as (")
    w("    insert into public.print_areas (business_id, name, code, is_active)")
    w("    select v_business, cfg.name, cfg.code, true")
    w("    from _area_cfg cfg")
    w("    where not exists (")
    w("      select 1 from public.print_areas a")
    w("      where a.business_id = v_business and a.code = cfg.code")
    ...
    w("    returning code")
    w("  )")
    w("  select string_agg(code, ', ' order by code) into v_creadas from ins;")
    w("  raise notice 'Areas creadas: %', coalesce(v_creadas, '(ninguna: ya existian, se reusan)');")
    w("end $$;")
    w("")
    w("-- ---------------------------------------------------------------------------")
    w("-- 2) Ruteo sku -> area")
    ...
    w("")
    w("-- Las areas que quedaron. impr_comandas = 0 -> el area acepta el producto")
    w("-- pero la comanda NO sale: hay que pegarle una impresora.")
    w("select a.code, a.name, a.is_active,")
    w("       (select count(*) from public.print_area_printers pp")
    w("         where pp.area_id = a.id) as impresoras,")
    w("       (select count(*) from public.print_area_printers pp")
    w("         where pp.area_id = a.id and pp.enabled and pp.prints_orders) as impr_comandas")
    w("from public.print_areas a")
    ...
    w("")
    w("-- ---------------------------------------------------------------------------")
    w("-- OPCIONAL — pegarle una impresora a un area, si no la configuras por la app.")
    w("-- Descomenta y pon el nombre exacto de la impresora y el code del area.")
    w("-- ---------------------------------------------------------------------------")
    w("-- insert into public.print_area_printers")
    w("--   (business_id, area_id, printer_id, priority, enabled, prints_orders, prints_prebills, prints_receipts)")
    w("-- select p.business_id, a.id, p.id, 1, true, true, false, false")
    w("-- from public.printers p")
    w("-- join public.print_areas a")
    w("--   on a.business_id = p.business_id and a.code = 'cocina'   -- << el area")
    w("-- where p.business_id = '%s'::uuid" % BID)
    w("--   and p.name = 'NOMBRE EXACTO DE LA IMPRESORA'             -- << la impresora")
    w("--   and not exists (select 1 from public.print_area_printers pp")
    w("--                    where pp.area_id = a.id and pp.printer_id = p.id);")
    w("")
