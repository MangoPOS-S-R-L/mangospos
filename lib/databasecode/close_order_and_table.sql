-- Close order and release table (set to available)
create or replace function public.fn_close_order_and_table(
  p_order_id uuid,
  p_status public.order_status -- 'paid' or 'void'
)
returns void
language plpgsql
security definer
...
grant execute on function public.fn_close_order_and_table(uuid, public.order_status)
  to authenticated;
