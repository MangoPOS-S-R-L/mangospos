-- ================================================================
-- COMPLETE SCHEMA REBUILD (v2)
-- Includes: Customers, Cashier, Kitchen, and Fixes for Enums
-- ================================================================

-- 1. FIX ENUM ISSUES (Critical for 'draft' error)
-- We need to ensure 'draft' is a valid status for order items.
DO $$ 
BEGIN
    ALTER TYPE item_status ADD VALUE IF NOT EXISTS 'draft';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not add draft to item_status, checking if it exists or if type is not distinct.';
END $$;
...
-- Clean up any conflicting triggers first to ensure fresh logic
DROP TRIGGER IF EXISTS trg_auto_kitchen ON order_items;
DROP FUNCTION IF EXISTS fn_auto_send_to_kitchen;

-- Ensure default status is 'draft' for order_items if not already set
-- Note: Logic to handle 'draft' error is at the top (ENUM update)
