-- ROLLBACK de 20260907_0003_consume_modifier_ingredients.sql
--
-- Restaura la versión de 20260901_0003 (almacenes por sección, SIN la rama de
-- modificadores y SIN el recorte a 0 del esperado).
--
-- OJO: si la base viva tenía algo que el repo no cuando se aplicó 0003, este
-- rollback tampoco lo tiene. Cotejar con pg_get_functiondef antes de correrlo.
--
-- El stub de fn_resolve_consumption_warehouse que 0003 pudo haber creado NO se
-- borra acá: es inofensivo (devuelve NULL = bodega por defecto) y borrarlo
-- rompería esta misma función. Para quitarlo, aplicar 20260901_0002 encima.
...
begin
  ...
  if v_src like '%UNIFICADA 20260915_0002%' or v_src like '%fn_resolve_area_warehouse%' or v_src like '%status_ext%' then
