...
--
-- QUÉ CARGA
--   54 productos · 4 categorías · 5 modificadores
--   SANDWICHERA 14  ·  JUGUERA 40
--
-- CÓMO CORRERLO
...
--   quedan exactamente 54 productos, todos con ITBIS y todos con área, el
...
--
-- IMPUESTOS: el menú dice "impuestos incluidos", así que los 54 entran con
...
begin
  ...
  if v_tax_rate <> 18 then
    raise exception
      ...
      'vincular 54 productos.', v_tax_name, v_tax_rate;
  end if;
  ...
  if v_group is null then
    ...
    values (v_group, v_business, 'Adicionales', 0, 5, true, 0);
  end if;
  ...
  if v_n <> 10 then
    raise exception
      'Se esperaban 10 sándwiches con adicionales y hay %. Revertido.', v_n;
  end if;

  raise notice 'OK — 54 productos, todos con ITBIS y área. Commit.';
end $$;
...
union all
select 'con área', count(distinct mipa.menu_item_id), 54
...
-- Reparto por área — esperado: JUGUERA 40 · SANDWICHERA 14
