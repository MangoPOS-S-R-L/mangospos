-- =============================================================================
-- Fix: get_sales_summary_v2 → product_sales venía con shape de top_products
-- (solo label/product_id/amount/net_sales/quantity), por lo que la UI de
-- "Ventas por producto" mostraba el literal "Producto", "Sin categoría" y
-- ceros en Cantidad/Brutas/Descuentos/Cortesías/Costo/Gan. bruta. Solo
-- aparecía el valor de Netas (que sí se emitía) y Proy. mes (calculado en
-- Flutter con product_id).
--
-- Esta migración:
--   1. Agrega un nuevo CTE `product_sales_agg` que agrupa por product_id
--      y expone product/category/quantity_sold/gross_sales/discounts/
--      courtesies/net_sales/tickets — exactamente lo que espera el shape
--      ProductSalesReportRow en reports_viewmodel.dart.
--   2. Reemplaza la emisión de `product_sales` para usar ese CTE.
--
-- Reutiliza la misma heurística de cortesía que `items_totals` (notes con
-- '[CORTESIA:' o discount >= gross_amount). cost/gross_profit se emiten
-- como 0: el POS no rastrea costo por línea (vive en inventory_items vía
-- recetas) y exponerlo requiere un cálculo aparte basado en BoM.
--
-- Brutas = SUM(adjusted_amount + discounts). Garantiza el cuadre
-- Brutas - Descuentos - Cortesías = Netas, donde adjusted_amount es el
-- valor con el delta de redondeo ya aplicado para que SUM por orden
-- empate con order_total.
-- =============================================================================
