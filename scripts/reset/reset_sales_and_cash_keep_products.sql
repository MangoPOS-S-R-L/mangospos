-- Reset de ventas + caja operativa
-- Conserva productos, catálogo, configuraciones y cajas (cash_registers).
-- Borra histórico operativo: ventas, pagos, checks, fiscal relacionado,
...
union all
select 'cash_register_sessions', count(*) from public.cash_register_sessions
union all
...
-- 2) Caja operativa / movimientos
-- Primero pagos porque pueden referenciar sesiones de caja
